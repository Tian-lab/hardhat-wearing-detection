...
ground_truth_files_list = glob.glob('./mAP/{}/*.txt'.format(gt_name))
...
for txt_file in ground_truth_files_list:
  ...
  if not os.path.exists('./mAP/' + pre_name + '/' + file_id + ".txt"):
    error_msg = "Error. File not found: ./mAP/" + pre_name + "/" +  file_id + ".txt\n"
...
predicted_files_list = glob.glob('./mAP/{}/*.txt'.format(pre_name))
...
for class_index, class_name in enumerate(gt_classes):
  ...
  for txt_file in predicted_files_list:
    ...
    file_id = os.path.basename(os.path.normpath(file_id))
    if class_index == 0:
      if not os.path.exists('./mAP/'+gt_name+'/' + file_id + ".txt"):
        error_msg = "Error. File not found: ./mAP/"+gt_name+"/" +  file_id + ".txt\n"
        error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
        error(error_msg)
